...
namespace ExitGames
{
	namespace Common
	{
		...
		public:
			...
			template<typename T> bool push(const T pData, short arraySize);
			bool push(const nByte* pData, int arraySize);
			bool push(nByte* pData, int arraySize);
		...
		template<typename T>
		bool Serializer::push(const T pData, short arraySize)
		{
			typename Helpers::ConfirmAllowed<T>::type forceCompilationToFailForUnsupportedTypes; (void)forceCompilationToFailForUnsupportedTypes;

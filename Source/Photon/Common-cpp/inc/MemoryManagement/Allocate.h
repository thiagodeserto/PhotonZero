...
/** @name LowLevelMemoryManagement
   ...
   The macros in this section are an alternative for the C dynamic memory management functions malloc(), free() and realloc().
   ...
   functions malloc(), free() and realloc() and the standard C++ operators new, new[], delete and delete[].*/
/*@{*/
...
/** @def EG_FREE
   Pass the address of memory, that has previously been returned by EG_MALLOC() or EG_REALLOC() to this function, to deallocate it.
   @details
   If the passed address is NULL, then this macro will do nothing.

   If a passed non-NULL address has not previously been returned by EG_MALLOC() or EG_REALLOC(), then the behavior is undefined. */
#define EG_FREE    ExitGames::Common::MemoryManagement::Internal::Interface::free

/** @def EG_REALLOC
   This macros resizes the block of memory at the passed address to the passed size and returns the new address of this block of memory.
   @details
   ...
   the block may includes moving it to a new location. When exactly a block gets moved, is an implementation detail, that could be
   different between implementations on different platforms and can be changed without notion. Especially block-movements can't only

...
namespace ExitGames
{
	namespace Common
	{
		...
		private:
			...
			template<typename Ftype> struct getDataCopyImplementation<Ftype*> {Ftype* operator()(const void* const pData, const typename Helpers::ArrayLengthType<Ftype*>::type* const pSizes, nByte type, unsigned int dimensions) const; private: void* dimensionRecursion(void* pArrayOut, const void* const pArrayIn, const typename Helpers::ArrayLengthType<Ftype*>::type* const pSizes, nByte type, unsigned int dimensions, unsigned int recursionDepth) const; void* lastDimension(const void* const ArrayIn, typename Helpers::ArrayLengthType<Ftype*>::type size) const;};
			template<typename Ftype> struct getDataAddressImplementation {Ftype* operator()(const void* const* const ppData) const;};
			template<typename Ftype> struct getDataAddressImplementation<Ftype*> {Ftype** operator()(const void* const* const ppData) const;};
		...
		template<typename Etype>
		ValueObject<Etype>::ValueObject(typename Helpers::ConfirmAllowed<Etype>::type const pData, const short* const sizes) : Object(pData, Helpers::ConfirmAllowed<Etype>::typeName, Helpers::ConfirmAllowed<Etype>::customTypeName, Helpers::ConfirmAllowed<Etype>::dimensions, sizes, true)
		{
		...
		/**
		   ...
		   All non-array copies free there memory automatically, as soon
		...
		void* ValueObject<Etype>::getDataCopyImplementation<Ftype*>::lastDimension(const void* const pArrayIn, typename Helpers::ArrayLengthType<Ftype*>::type size) const
		{
			...
			typename Helpers::ConfirmAllowed<Ftype*>::scalarType* pRetVal = MemoryManagement::allocateArray<typename Helpers::ConfirmAllowed<Ftype*>::scalarType>(size);
